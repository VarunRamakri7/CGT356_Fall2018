...
<body>
    <h1 style="text-align:center;">Billing Update Page</h1>
    <?php 
        ...
        $sql="SELECT DISTINCT uniqueid, firstname, lastname, cardnumber, cardtype, expiration, ccv, address, state, city, zip, country FROM PaymentInformation WHERE username=".$username;
            ...
            <fieldset id="billing">
                ...
                <ul>
                    ...
                    <li> <label title="Cardnumber" for="cardnumber">Cardnumber<span>*</span></label>
                         <input type="text" name="cardnumber" id="cardnumber" size="30" maxlength="30"  value = "
                            <?php 
                                if($result->num_rows != 0) 
                                { 
                                    echo(trim($row["Cardnumber"])); 
                                } 
                                ?>" /></li>
                    <li> <label title="Cardtype" for="cardtype">Cardtype<span>*</span></label>
                         <input type="text" name="cardtype" id="cardtype" size="30" maxlength="30"  value = "
                            <?php 
                                if($result->num_rows != 0) 
                                { 
                                    echo(trim($row["Cardtype"])); 
                                } 
                            ?>" /></li>
                    <li> <label title="Expiration" for="expiration">Expiration<span>*</span></label>
                         <input type="text" name="expiration" id="expiration" size="30" maxlength="30"  value = "
                            <?php 
                                if($result->num_rows != 0) 
                                { 
                                    echo(trim($row["Expiration"])); 
                                } 
                            ?>" /></li>
                    <li> <label title="CCV" for="ccv">CCV<span>*</span></label>
                         <input type="text" name="ccv" id="ccv" size="30" maxlength="30"  value = "
                            <?php 
                                if($result->num_rows != 0) 
                                { 
                                    echo(trim($row["CCV"])); 
